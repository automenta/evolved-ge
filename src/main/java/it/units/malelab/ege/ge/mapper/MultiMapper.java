      ...
      case RESERVED_BITS:
        {
          ...
          innerGenotype = genotype.slice(mapperIndex, genotype.leaves());
          break;
        }
      case ALL_MODULE:
      ...
      case ALL_BINARY:
        {
          int mapperBits = (int) Math.ceil(Math.log10(mappers.size()) / Math.log10(2d));
          int index = 0;
          List<BitsGenotype> slices = genotype.slices(Utils.slices(Range.closedOpen(0, genotype.leaves()), mapperBits));
          for (int i = 0; i<mapperBits; i++) {
            int value = (int)Math.round((double)slices.get(i).count()/ slices.get(i).leaves());
      ...
      case ALL_MAX:
        {
          ...
          List<BitsGenotype> slices = genotype.slices(Utils.slices(Range.closedOpen(0, genotype.leaves()), mappers.size()));
          for (int i = 0; i<slices.size(); i++) {
            double value = (double)slices.get(i).count()/ slices.get(i).leaves();
            if (value>maxValue) {
              maxValue = value;
              maxIndex = i;
            }
